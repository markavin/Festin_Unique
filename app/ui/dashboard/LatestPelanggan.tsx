import { ArrowPathIcon } from '@heroicons/react/24/outline';
import clsx from 'clsx';
import { fetchLatestPelanggan } from '@/app/lib/data';
import { kanit } from '@/app/ui/fonts';
import { bebas_Neue } from '../fontz';


export default async function LatestPelanggan() {
    const latestPelanggan = await fetchLatestPelanggan();


    return (
        <div className="flex w-full flex-col md:col-span-4">
            <h2 className={`${bebas_Neue.className} mb-4 text-xl md:text-2xl`}>
                Latest Customers
            </h2>
            <div className="flex grow flex-col justify-between rounded-xl bg-gradient-to-b from-red-900 to-gray-950 p-4">
                <div className="bg-white px-6 py-4 overflow-y-auto rounded-lg">
                    {latestPelanggan.map((pelanggan, i) => (
                        <div
                            key={pelanggan.id}
                            className={clsx(
                                'flex flex-row items-center justify-between py-4',
                                {
                                    'border-t': i !== 0,
                                },
                            )}
                        >
                            <div className="flex items-center">
                                <div className="min-w-0">
                                    <p className="truncate text-sm font-semibold md:text-base">
                                        {pelanggan.name}
                                    </p>
                                    <p className="text-xs text-gray-500 truncate">
                                        {pelanggan.email}
                                    </p>
                                </div>
                            </div>
                            <p
                                className={`${kanit.className} text-sm font-medium md:text-base text-left`}
                            >
                                {pelanggan.nohp}
                            </p>
                        </div>
                    ))}
                </div>
                <div className="flex items-center pb-2 pt-6">
                    <ArrowPathIcon className="h-5 w-5 text-gray-500" />
                    <h3 className="ml-2 text-sm text-gray-500">Updated just now</h3>
                </div>
            </div>
        </div>
    );
}





